...
def createEvent(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            print("Test 2")
            warning("before", form.cleaned_data['start_date'])
            warning("after", form.cleaned_data['start_date'])
            form.save()
            warning("after save", form.cleaned_data['start_date'])
            return HttpResponseRedirect(reverse('calendar:month_shift'))
        else:
            print("Test 3")
            print('Form errors', form.errors)
            form = EventForm()

        return render(request, 'happenings/createevent.html', {'form': form})
    else:
        print("Test 4")
        form = EventForm()
...
def editEvent(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            post.start_onlydate = form.cleaned_data.get('start_onlydate')
            post.end_onlydate = form.cleaned_data.get('end_onlydate')
            post.start_onlytime = form.cleaned_data.get('start_onlytime')
            post.end_onlytime = form.cleaned_data.get('end_onlytime')
                        
            post.phone = form.cleaned_data.get('phone')
            post.address_1 = form.cleaned_data.get('address_1')
            post.address_2 = form.cleaned_data.get('address_2')
            post.city = form.cleaned_data.get('city')
            post.state = form.cleaned_data.get('state')
            post.zipcode = form.cleaned_data.get('zipcode')
            post.sales_order = form.cleaned_data.get('sales_order')
            post.pick = form.cleaned_data.get('pick')
            post.weight = form.cleaned_data.get('weight')
            post.pallet = form.cleaned_data.get('pallet')
            ...
            print("test 3")
            print('Form errors', form.errors)
            return render(request, 'happenings/editevent.html', locals())
    else:
        print("test 4")
